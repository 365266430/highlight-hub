...
- 后端集成测试（真实 MySQL）：`cd backend && mvn test`（43 个用例）
- Worker 单元/夹具测试：`cd media-worker && pytest`（12 个用例，合成夹具）
- 端到端闭环：`scripts/run_e2e.py`（阶段1，31 项）+ `scripts/run_e2e_phase2.py`（阶段2，13 项）
