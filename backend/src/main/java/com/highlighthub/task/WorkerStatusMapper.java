...
public interface WorkerStatusMapper {
    ...
    @Insert("""
            INSERT INTO worker_status (worker_id, last_heartbeat, active_task_id, created_at, updated_at)
            VALUES (#{workerId}, UTC_TIMESTAMP(3), #{activeTaskId}, UTC_TIMESTAMP(3), UTC_TIMESTAMP(3))
            ON DUPLICATE KEY UPDATE last_heartbeat = UTC_TIMESTAMP(3),
              active_task_id = VALUES(active_task_id), updated_at = UTC_TIMESTAMP(3)
            """)
