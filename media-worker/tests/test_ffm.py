...
def synthetic_clip() -> str:
    """10s 640x360 30fps clip with test pattern + beep track (SYNTHETIC TEST)."""
    ...
    args = [
        FFMPEG, "-y",
        "-f", "lavfi", "-i", f"testsrc2=size=640x360:rate=30:duration=10",
...
def silent_clip(synthetic_clip) -> str:
    """A no-audio variant derived from the synthetic clip."""
